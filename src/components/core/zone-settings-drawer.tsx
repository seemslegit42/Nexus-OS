...
export function ZoneSettingsDrawer({
  ...
  const [isVisible, setIsVisible] = React.useState(currentSettings.isVisible);
  const [linkedAgent, setLinkedAgent] = React.useState(currentSettings.linkedAgent || 'None');
  const [scheduleTime, setScheduleTime] = React.useState(currentSettings.scheduleTime || '');
  const [notificationsEnabled, setNotificationsEnabled] = React.useState(currentSettings.notificationsEnabled);
    ...
    <Sheet open={!!zoneId} onOpenChange={onOpenChange}>
      <SheetContent className="flex flex-col">
        <SheetHeader>
          ...
          <SheetDescription>
            Configure settings for this workspace zone. Changes are saved locally.
          </SheetDescription>
        </SheetHeader>
        <div className="grid gap-6 py-4 flex-grow">
          ...
          <div className="space-y-1.5">
            <Label htmlFor={`linked-agent-${zoneId}`} className="text-sm">Linked Agent</Label>
            <Select value={linkedAgent} onValueChange={setLinkedAgent}>
              <SelectTrigger id={`linked-agent-${zoneId}`} className="bg-input border-input focus:ring-primary">
          ...
          <div className="space-y-1.5">
            <Label htmlFor={`schedule-time-${zoneId}`} className="text-sm">Schedule Automation (Time)</Label>
            <Input
