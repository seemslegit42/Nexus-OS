...
export function WorkspaceGrid({
  ...
  const [storedLayoutsBeforeMaximize, setStoredLayoutsBeforeMaximize] =
    useState<Layouts | null>(null);

  const [minimizedZoneIds, setMinimizedZoneIds] = useState<string[]>([]);
  const [storedHeightsBeforeMinimize, setStoredHeightsBeforeMinimize] =
    useState<Record<string, { h: number; minH: number }>>({});
  const [locallyClosedZoneIds, setLocallyClosedZoneIds] = useState<string[]>(
    []
  );

  const [dynamicRowHeight, setDynamicRowHeight] = useState(
    rowHeight || DEFAULT_ROW_HEIGHT_PIXELS
  );

  const [zoneSettingsMap, setZoneSettingsMap] = useState<
    Record<string, ZoneSpecificSettings>
  >({});
  const [editingSettingsForZoneId, setEditingSettingsForZoneId] = useState<
    string | null
  >(null);
  ...
  const handleOpenApp = useCallback(
    (zoneId: string, zoneTitle: string) => {
      addLog(
        `'Open App' action triggered for zone: ${zoneTitle} (ID: ${zoneId})`,
        'WorkspaceAction'
      );
  ...
  const handleRunTask = useCallback(
    async (zoneId: string, zoneTitle: string) => {
      addLog(
        `'Run Task Now' initiated for zone: ${zoneTitle} (ID: ${zoneId})`,
        'WorkspaceAction'
      );
      // Remove artificial delay - use proper async pattern
      addLog(
        `Task completed for zone: ${zoneTitle} (ID: ${zoneId})`,
        'WorkspaceAction'
      );
    },
    [addLog]
  ...
  const handleViewLogs = useCallback(
    (zoneId: string, zoneTitle: string) => {
      addLog(
        `'View Logs' action triggered for zone: ${zoneTitle} (ID: ${zoneId})`,
        'WorkspaceAction'
      );
      alert(
        `Simulating viewing logs for: ${zoneTitle}. Check persistent console.`
      );
    },
    [addLog]
  );
  ...
  const memoizedZoneConfigs = useMemo(() => {
    return initialZoneConfigs.map(zc => ({
      ...zc,
      onOpenApp: zc.onOpenApp || handleOpenApp,
      onRunTask: zc.onRunTask || handleRunTask,
      onViewLogs: zc.onViewLogs || handleViewLogs,
    }));
  }, [initialZoneConfigs, handleOpenApp, handleRunTask, handleViewLogs]);

  const activeZoneConfigs = useMemo(() => {
    return memoizedZoneConfigs.filter(
      zc =>
        !locallyClosedZoneIds.includes(zc.id) &&
        (zoneSettingsMap[zc.id]?.isVisible ?? true)
    );
  }, [memoizedZoneConfigs, locallyClosedZoneIds, zoneSettingsMap]);

  useEffect(() => {
    if (!rowHeight) {
      // Only calculate if not overridden
      const dummyHeader = document.createElement('div');
      dummyHeader.className =
        'draggable-zone-header flex flex-row items-center justify-between space-y-0 p-3 border-b border-border/50 min-h-[48px]';
      ...
      setDynamicRowHeight(
        Math.max(10, Math.ceil(headerPixelHeight / MINIMIZED_ZONE_HEADER_ROWS))
      );
    }
  }, [rowHeight]);
  ...
  useEffect(() => {
    ...
    memoizedZoneConfigs.forEach(zc => {
      initialSettings[zc.id] = {
        ...
        notificationsEnabled:
          zc.defaultZoneSettings?.notificationsEnabled ?? true,
        hasActiveAutomation: false,
      };
      initialSettings[zc.id].hasActiveAutomation = !!(
        initialSettings[zc.id].scheduleTime ||
        initialSettings[zc.id].linkedAgent
      );
    });
  ...
  const initializeDefaultLayouts = useCallback(() => {
    ...
    Object.keys(cols).forEach(bpKey => {
      ...
      initialLayouts[bp] = memoizedZoneConfigs
        .filter(
          zc =>
            !locallyClosedZoneIds.includes(zc.id) &&
            (zoneSettingsMap[zc.id]?.isVisible ?? true)
        ...
        .map(zc => {
          ...
          const itemStatic = zc.static ?? false;
          const itemIsResizable = zc.isResizable ?? !itemStatic;
          const itemIsDraggable = zc.isDraggable ?? !itemStatic;
      ...
      if (savedLayouts) {
        try {
          ...
          if (
            ...
            parsedLayouts !== null &&
            Object.keys(parsedLayouts).some(key => cols.hasOwnProperty(key))
          ) {
            setCurrentLayouts(parsedLayouts);
          } else {
            console.warn(
              `Invalid layouts found in localStorage for key "${storageKey}". Using defaults.`
            );
            initializeDefaultLayouts();
          }
        } catch (e) {
          console.error(
            `Error parsing layouts from localStorage for key "${storageKey}":`,
            e
          );
          initializeDefaultLayouts();
        }
  ...
  const updateLayoutItemForAllBreakpoints = useCallback(
    (id: string, newProps: Partial<Layout>) => {
  ...
  const handleTogglePin = useCallback(
    ...
    [
      ...
      maximizedZoneId,
      minimizedZoneIds,
      updateLayoutItemForAllBreakpoints,
    ]
  );
    ...
    (id: string) => {
      setCurrentLayouts(prevLayouts => {
        ...
        if (maximizedZoneId === id) {
          ...
          Object.keys(newLayoutsState).forEach(bp => {
            if (newLayoutsState[bp]) {
              (newLayoutsState[bp] as Layout[]).forEach((itemL: Layout) => {
                const wasMinimizedBeforeMaximize =
                  minimizedZoneIds.includes(itemL.i) &&
                  storedLayoutsBeforeMaximize?.[bp]?.find(l => l.i === itemL.i);
          ...
          setStoredLayoutsBeforeMaximize(
            JSON.parse(JSON.stringify(prevLayouts))
          );
          setMaximizedZoneId(id);
          const updatedMinimizedZoneIds = minimizedZoneIds.filter(
            minId => minId !== id
          );
            ...
            if (bpLayout) {
              bpLayout.forEach((itemL: Layout) => {
                if (itemL.i === id) {
                  ...
                  const viewportHeightInRows = Math.floor(
                    window.innerHeight / (dynamicRowHeight + 5)
                  );
                  itemL.h = Math.max(
                    10,
                    viewportHeightInRows - MINIMIZED_ZONE_HEADER_ROWS - 1
                  );
  ...
  const handleToggleMinimize = useCallback(
    (id: string) => {
      ...
      const itemCurrentLayoutForBp = currentLayouts[currentBreakpoint]?.find(
        l => l.i === id
      );
      ...
      if (!isCurrentlyMinimized) {
        ...
        setStoredHeightsBeforeMinimize(prev => ({
          ...prev,
          [id]: {
            h: itemCurrentLayoutForBp.h,
            minH: itemCurrentLayoutForBp.minH || MINIMIZED_ZONE_HEADER_ROWS,
          },
        }));
        updateLayoutItemForAllBreakpoints(id, {
        ...
        updateLayoutItemForAllBreakpoints(id, {
          ...
          minH:
            oldHeights?.minH ||
            itemCurrentLayoutForBp.minH ||
            MINIMIZED_ZONE_HEADER_ROWS,
  ...
  const handleActualClose = useCallback(
    (id: string) => {
      if (maximizedZoneId) return;
      addLog(
        `Zone '${memoizedZoneConfigs.find(zc => zc.id === id)?.title || id}' removed.`,
        'WorkspaceAction'
      );
      if (onZoneClose) {
        onZoneClose(id);
    ...
    [maximizedZoneId, addLog, memoizedZoneConfigs, onZoneClose]
  );
  ...
  const handleSaveZoneSettings = useCallback(
    (zoneId: string, newSettings: ZoneSpecificSettings) => {
      const derivedHasActiveAutomation = !!(
        newSettings.scheduleTime ||
        (newSettings.linkedAgent && newSettings.linkedAgent !== 'None')
      );
      setZoneSettingsMap(prev => ({
        ...prev,
        [zoneId]: {
          ...newSettings,
          hasActiveAutomation: derivedHasActiveAutomation,
        },
      }));
      if (!newSettings.isVisible && !locallyClosedZoneIds.includes(zoneId)) {
        ...
        newSettings.isVisible &&
        locallyClosedZoneIds.includes(zoneId)
      ) {
        if (!onZoneClose) {
      ...
      addLog(
        `Settings saved for zone: ${memoizedZoneConfigs.find(zc => zc.id === zoneId)?.title || zoneId}`,
        'WorkspaceAction'
      );
    },
    [addLog, memoizedZoneConfigs, onZoneClose, locallyClosedZoneIds]
  );
  ...
  const currentEditingZoneConfig = memoizedZoneConfigs.find(
    zc => zc.id === editingSettingsForZoneId
  );
    ...
    <>
      ...
      >
        {activeZoneConfigs.map(zoneConfig => {
          const rglItem =
            layoutsToRender[currentBreakpoint]?.find(
              item => item.i === zoneConfig.id
            ) ||
            currentLayouts[currentBreakpoint]?.find(
              item => item.i === zoneConfig.id
            ) ||
          ...
          const isEffectivelyPinned =
            zoneConfig.static ||
            (rglItem && 'static' in rglItem ? rglItem.static : false) ||
            false;
          ...
          const canPinZone =
            zoneConfig.canPin !== false && !isCurrentlyMaximized;
          const canMaximizeZone =
            zoneConfig.canMaximize !== false &&
            !isActuallyMinimized &&
            (!maximizedZoneId || isCurrentlyMaximized);
          const canMinimizeZone =
            zoneConfig.canMinimize !== false && !isCurrentlyMaximized;
          const canCloseZone =
            zoneConfig.canClose !== false && !isCurrentlyMaximized;
          const canSettingsZone =
            zoneConfig.canSettings !== false && !isCurrentlyMaximized;
          const currentZoneSettings = zoneSettingsMap[zoneConfig.id];

          if (!rglItem) {
            console.warn(
              `Layout item for zone ${zoneConfig.id} not found for breakpoint ${currentBreakpoint}. Skipping render.`
            );
            return null;
          }
          ...
          return (
            ...
            >
              <Zone
                ...
                hasActiveAutomation={
                  currentZoneSettings?.hasActiveAutomation ?? false
                }
      ...
      {editingSettingsForZoneId && currentEditingZoneConfig && (
        <ZoneSettingsDrawer
          ...
          onOpenChange={open => {
            if (!open) setEditingSettingsForZoneId(null);
          }}
