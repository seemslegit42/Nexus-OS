...
interface MicroAppDetailDrawerProps {
  ...
  availableAgents: string[]; // For agent dependency selection
}
...
export function MicroAppDetailDrawer({ app, isOpen, onOpenChange, onSave, availableAgents }: MicroAppDetailDrawerProps) {
  ...
  const handleSaveClick = () => {
    onSave(formData as MicroApp); // Assume formData is valid for now
    onOpenChange(false);
  };
      ...
      <SheetContent className="w-full sm:max-w-lg flex flex-col p-0">
        <SheetHeader className="p-4 border-b border-border/60">
          ...
          <SheetDescription className="text-xs">
            Internal ID: {app.id} | Version: {app.version}
          </SheetDescription>
        </SheetHeader>
                ...
                <AccordionItem value="agents">
                    ...
                    <AccordionContent className="pt-2 pb-0 px-1">
                        <div className="grid gap-1.5">
                            ...
                            {/* Basic multi-select placeholder, a real component would be better */}
                            <Textarea 
                            ...
                            />
                            <p className="text-xs text-muted-foreground">Available: {availableAgents.join(', ')}</p>
                        </div>
                    </AccordionContent>
                </AccordionItem>
