...
const SecuritySurfaceScanApp: React.FC = () => {
  ...
  const isValidTarget = (target: string): boolean => {
    ...
    if (!target.includes('.')) {
      setError(
        'Target must be a valid domain or URL (e.g., example.com or http://example.com).'
      );
      return false;
    }
    ...
    if (!/^(?:[a-zA-Z]+:\/\/)/.test(target) && target.includes('/')) {
      setError(
        'Invalid URL format. If including a path, ensure a protocol (e.g., http://) is present.'
      );
      return false;
    }
    return true;
    ...
    >
      ...
      <form onSubmit={handleScanSubmit} style={{ marginTop: '20px' }}>
        ...
        {error && (
          <div
            id="target-error"
            style={{ color: 'red', fontSize: '0.9em', marginBottom: '10px' }}
          >
            {error}
          </div>
        )}
