...
const DeployAgentSchema = z.object({
  ...
  userId: z.number().int().positive("User ID must be a positive integer.").default(1), 
...
export async function deployAgentInstance(
  ...
  try {
    ...
    if (!validatedInput.success) {
      return { success: false, message: validatedInput.error.flatten().fieldErrors.toString() };
    }

    const { marketplaceAgentId, instanceName, instanceDescription, userId, config } = validatedInput.data;
    ...
    const newInstance = await db
      ...
      .values({
        ...
        metadata: instanceDescription ? { description: instanceDescription } : undefined,
        config: config,
    ...
    if (newInstance && newInstance.length > 0) {
      ...
      return { success: true, instanceId: newInstance[0].id, message: "Agent instance deployment initiated." };
    } else {
      return { success: false, message: 'Failed to create agent instance in the database.' };
    }
  } catch (error) {
