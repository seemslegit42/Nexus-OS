...
export const users = pgTable('users', {
  ...
  email: varchar('email', { length: 256 }).unique('users_email_unique').notNull(), // Unique email constraint
...
export const workspaces = pgTable('workspaces', {
  ...
  unqNamePerOwner: unique('workspaces_name_owner_id_unique').on(table.name, table.ownerId),
...
export const workspaceMembers = pgTable('workspace_members', {
  ...
  unqUserWorkspace: unique('workspace_members_workspace_id_user_id_unique').on(table.workspaceId, table.userId),
...
export const projects = pgTable('projects', {
  ...
  unqNamePerWorkspace: unique('projects_name_workspace_id_unique').on(table.name, table.workspaceId),
...
export const deployedAgentInstances = pgTable('deployed_agent_instances', {
  ...
  status: varchar('status', { length: 50 }).notNull().default('pending_config'),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
...
export const agentSessions = pgTable('agent_sessions', {
  id: serial('id').primaryKey(),
  agentInstanceId: integer('agent_instance_id').references(() => deployedAgentInstances.id, { onDelete: 'cascade' }).notNull(),
  sessionData: jsonb('session_data').default({}).notNull(), // Default to empty JSON object
  startedAt: timestamp('started_at').defaultNow().notNull(),
  endedAt: timestamp('ended_at'), // Nullable
}, (table) => ({
  // Index for foreign key
  agentInstanceIdIdx: index('agent_sessions_agent_instance_id_idx').on(table.agentInstanceId),
}));

// ---------- LOOM STUDIO (VISUAL FLOWS) ----------
export const loomFlows = pgTable('loom_flows', {
  id: serial('id').primaryKey(),
  ...
  unqNamePerProject: unique('loom_flows_name_project_id_unique').on(table.name, table.projectId),
  // Index for foreign key
  projectIdIdx: index('loom_flows_project_id_idx').on(table.projectId),
}));

export const loomNodes = pgTable('loom_nodes', {
  id: serial('id').primaryKey(),
  flowId: integer('flow_id').references(() => loomFlows.id, { onDelete: 'cascade' }).notNull(),
...
export const loomEdges = pgTable('loom_edges', {
  id: serial('id').primaryKey(),
  flowId: integer('flow_id').references(() => loomFlows.id, { onDelete: 'cascade' }).notNull(),
  sourceNodeId: integer('source_node_id').references(() => loomNodes.id, { onDelete: 'cascade' }).notNull(), // Must have a source node
  targetNodeId: integer('target_node_id').references(() => loomNodes.id, { onDelete: 'cascade' }).notNull(), // Must have a target node
  label: text('label'), // Nullable
}, (table) => ({
  // Composite unique index to prevent duplicate edges within a flow
  unqEdgePerFlow: unique('loom_edges_flow_source_target_unique').on(table.flowId, table.sourceNodeId, table.targetNodeId),
  // Indexes for foreign keys
  flowIdIdx: index('loom_edges_flow_id_idx').on(table.flowId),
  sourceNodeIdIdx: index('loom_edges_source_node_id_idx').on(table.sourceNodeId),
  targetNodeIdIdx: index('loom_edges_target_node_id_idx').on(table.targetNodeId),
}));

// ---------- MARKETPLACE AGENTS ----------
export const marketplaceAgents = pgTable('marketplace_agents', {
  id: serial('id').primaryKey(),
  name: varchar('name', { length: 255 }).unique('marketplace_agents_name_unique').notNull(), // Agent names should be unique
  description: text('description'), // Nullable
  configSchema: jsonb('config_schema').default({}).notNull(), // Schema for agent config required
  createdBy: integer('created_by').references(() => users.id, { onDelete: 'set null' }), // Nullable
  createdAt: timestamp('created_at').defaultNow().notNull(),
}, (table) => ({
  // Index for foreign key
  createdByIdx: index('marketplace_agents_created_by_idx').on(table.createdBy),
}));

// ---------- PROMPT LOGS & BILLING ----------
export const promptLogs = pgTable('prompt_logs', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'set null' }), // Nullable if system prompts or anonymous
...
export const auditLogs = pgTable('audit_logs', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'set null' }), // Nullable if system actions
...
export const notifications = pgTable('notifications', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'cascade' }).notNull(), // Notification must be for a user
  workspaceId: integer('workspace_id').references(() => workspaces.id, { onDelete: 'cascade' }), // Nullable for global notifications
  type: varchar('type', { length: 100 }).notNull(),
...
export const microApps = pgTable('micro_apps', {
  id: serial('id').primaryKey(),
  name: varchar('name', { length: 255 }).unique('micro_apps_name_unique').notNull(),
  slug: varchar('slug', { length: 255 }).unique('micro_apps_slug_unique').notNull(),
...
// ---------- AGENT EVENT LOGS ----------
export const agentEventLogs = pgTable('agent_event_logs', {
  id: serial('id').primaryKey(),
  agentInstanceId: integer('agent_instance_id').references(() => deployedAgentInstances.id, { onDelete: 'cascade' }).notNull(),
  ...
  agentInstanceIdIdx: index('agent_event_logs_agent_instance_id_idx').on(table.agentInstanceId),
...
export const runtimeEnvironments = pgTable('runtime_environments', {
  id: serial('id').primaryKey(),
  agentInstanceId: integer('agent_instance_id').references(() => deployedAgentInstances.id, { onDelete: 'cascade' }).notNull(),
  ...
  // Index for foreign key
  agentInstanceIdIdx: index('runtime_environments_agent_instance_id_idx').on(table.agentInstanceId),
}));
...
export const rolePermissions = pgTable('role_permissions', {
  ...
  unqPermissionPerRole: unique('role_permissions_role_id_permission_unique').on(table.roleId, table.permission),
  // Index for foreign key
...
export const userSettings = pgTable('user_settings', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'cascade' }).unique('user_settings_user_id_unique').notNull(), // One-to-one with users
...
export const workspaceSettings = pgTable('workspace_settings', {
  id: serial('id').primaryKey(),
  workspaceId: integer('workspace_id').references(() => workspaces.id, { onDelete: 'cascade' }).unique('workspace_settings_workspace_id_unique').notNull(), // One-to-one with workspaces
  preferences: jsonb('preferences').default({}).notNull(),
  featureFlags: jsonb('feature_flags').default({}).notNull(),
  updatedAt: timestamp('updated_at').defaultNow().notNull(),
}, (table) => ({
...
export const fileUploads = pgTable('file_uploads', {
  id: serial('id').primaryKey(),
  userId: integer('user_id').references(() => users.id, { onDelete: 'set null' }), // Nullable if uploaded by system/agent
  workspaceId: integer('workspace_id').references(() => workspaces.id, { onDelete: 'cascade' }).notNull(), // Must belong to a workspace
  projectId: integer('project_id').references(() => projects.id, { onDelete: 'cascade' }), // Nullable if not project-specific
  ...
  workspaceIdIdx: index('file_uploads_workspace_id_idx').on(table.workspaceId),
...
export const userRoles = pgTable('user_roles', {
  ...
  unqUserRoleScope: unique('user_roles_user_workspace_role_unique').on(table.userId, table.workspaceId, table.roleId),
  // Indexes for foreign keys
