...
const generateTeamMembers = (): TeamMember[] => [
  ...
  { id: 'user_3', name: 'Sam Chen', email: 'sam.chen@company.com', role: 'Developer', avatar: 'https://placehold.co/40x40.png?text=SC', dataAiHint:'user avatar' },
];
...
const generateAdminUsers = (): AdminUser[] => [
  ...
  { id: 'user_3', name: 'Casey Smith', email: 'casey@test.com', role: 'User', status: 'Pending', lastLogin: 'Never', avatar: 'https://placehold.co/40x40.png?text=CS', dataAiHint: 'user avatar' },
];
...
const generateReportedContent = (filterStatus: string): ReportedContent[] => {
  const allReports: ReportedContent[] = [
    ...
    { id: 'report_3', itemId: 'item_Z', itemName: 'Inappropriate Agent Name', itemType: 'Agent', reporter: 'UserGamma', reason: 'Agent name contains offensive language.', reportedDate: '2023-10-24', status: 'Resolved - Warning Issued' },
  ];
  ...
  return filterStatus === 'all' 
    ...
    : allReports.filter(report => {
        ...
        if (filterStatus === 'resolved') return report.status.includes('Resolved');
        return true;
      });
};
...
const generateSubmissions = (filterStatus: string): Submission[] => {
  const allSubmissions: Submission[] = [
    ...
    { id: 'sub_003', itemName: 'Weather Forecast Agent', itemType: 'Agent', submitter: 'WeatherWiz', submittedDate: '2023-10-23', status: 'Rejected - Quality Issues' },
  ];
  ...
  return filterStatus === 'all' 
    ...
    : allSubmissions.filter(submission => {
        ...
        if (filterStatus === 'approved') return submission.status === 'Approved';
        if (filterStatus === 'rejected') return submission.status.includes('Rejected');
        return true;
      });
};

const generateIntegrations = (): Integration[] => [
  { id: 'slack', name: 'Slack', description: 'Team collaboration and messaging', status: 'Connected', icon: 'MessageSquare', connectedAt: '2023-10-15' },
  { id: 'github', name: 'GitHub', description: 'Code repository and version control', status: 'Connected', icon: 'Github', connectedAt: '2023-09-20' },
  { id: 'google-drive', name: 'Google Drive', description: 'Cloud storage and file sharing', status: 'Disconnected', icon: 'HardDrive' },
  { id: 'discord', name: 'Discord', description: 'Community and voice chat', status: 'Disconnected', icon: 'MessageCircle' },
];
...
const generateEarningsData = (): EarningsData => ({
  ...
  lastMonth: 523.25,
  transactionHistory: [
    { id: 'txn_001', type: 'Agent Sale', amount: 29.99, date: '2023-10-26', description: 'DataMiner Pro Agent' },
    { id: 'txn_002', type: 'Template License', amount: 15.50, date: '2023-10-25', description: 'Dashboard Template' },
    { id: 'txn_003', type: 'Subscription Revenue', amount: 199.00, date: '2023-10-24', description: 'Pro Plan (Monthly)' },
    { id: 'txn_004', type: 'Module Commission', amount: 12.25, date: '2023-10-23', description: 'Authentication Module' },
  ],
});

// Data fetchers with caching and proper async patterns
export const fetchUserProfile = async (): Promise<UserProfile> => {
  const cached = dataCache.get<UserProfile>(CACHE_KEYS.USER_PROFILE);
  if (cached) return cached;

  // Simulate real API call with minimal delay
...
export const fetchReportedContent = async (filterStatus: string): Promise<ReportedContent[]> => {
...
export const fetchSubmissions = async (filterStatus: string): Promise<Submission[]> => {
