...
const SuggestAgentTaskInputSchema = z.object({
  userProfile: z
    .string()
    .describe(
      'A description of the user profile, including their skills, interests, and goals.'
    ),
});
...
const SuggestAgentTaskOutputSchema = z.object({
  taskSuggestion: z
    .string()
    .describe('A suggestion for a task that the agent can perform.'),
  reasoning: z.string().describe('The reasoning behind the task suggestion.'),
});
export type SuggestAgentTaskOutput = z.infer<
  typeof SuggestAgentTaskOutputSchema
>;

export async function suggestAgentTask(
  input: SuggestAgentTaskInput
): Promise<SuggestAgentTaskOutput> {
