...
const STATUS_CYCLE: MicroApp['status'][] = [
  'dev-only',
  'beta',
  'enabled',
  'disabled',
...
export interface MicroAppRegistryState {
  ...
  registerMicroApp: (
    newAppData: Partial<
      Omit<MicroApp, 'id' | 'createdAt' | 'updatedAt' | 'defaultLayout'>
    > & {
      internalName: string;
      displayName: string;
      defaultLayout?: MicroApp['defaultLayout'];
    }
  ) => void;
  ...
  (set, get) => ({
    ...
    registerMicroApp: newAppData => {
      const newApp: MicroApp = {
        ...
        description:
          newAppData.description || 'A new micro-application for NexOS.',
        ...
        authRequired:
          newAppData.authRequired !== undefined
            ? newAppData.authRequired
            : true,
        isVisible:
          newAppData.isVisible !== undefined ? newAppData.isVisible : true,
        requiresSubscription:
          newAppData.requiresSubscription !== undefined
            ? newAppData.requiresSubscription
            : newAppData.monetization?.enabled || false,
        monetization:
          newAppData.monetization !== undefined
            ? newAppData.monetization
            : {
                enabled: false,
                price: undefined,
        ...
        ...newAppData, // Spread other properties - this will override the above defaults
      };
      set(state => ({
      ...
      return get().apps.filter(
        app =>
          ...
          (app.agentDependencies &&
            app.agentDependencies.some(agent =>
              agent.toLowerCase().includes(lowercasedTerm)
            ))
      );
    },
    getAppsByFlag: flagName => {
      ...
      return allApps.filter(
        app =>
          app.flags && app.flags[flagName as keyof MicroApp['flags']] === true
      );
    },
    getDeployableApps: () => {
      return get().apps.filter(
        app =>
          ...
          app.isVisible === true &&
          !(
            app.flags?.systemInternal &&
            (app.category === 'Security' || app.category === 'Core OS')
          ) &&
