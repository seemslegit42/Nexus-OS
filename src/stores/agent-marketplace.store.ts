...
export const useAgentMarketplaceStore = create<AgentMarketplaceState>((set, get) => ({
  ...
  searchAgents: (term) => {
    ...
    return get().agents.filter(agent =>
      ...
      (agent.tags && agent.tags.some(tag => tag.toLowerCase().includes(lowercasedTerm))) ||
      (agent.author.toLowerCase().includes(lowercasedTerm))
    );
  },
  getAgentsByCategory: (category) => {
    const lowercasedCategory = category.toLowerCase();
    return get().agents.filter(agent => agent.category.toLowerCase() === lowercasedCategory);
  },
}));
