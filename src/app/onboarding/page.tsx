    ...
    <Card className="h-full bg-transparent border-none shadow-none flex items-center justify-center"> {/* Centering content */}
      <CardContent className="p-3 sm:p-6 max-w-2xl w-full">
        ...
        {currentStep === 2 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input id="fullName" placeholder="e.g., Alex Ryder" className="mt-1 bg-background/70 border-input focus:ring-primary h-9 text-sm" />
            </div>
            <div>
              <Label htmlFor="role" className="text-xs">Your Role</Label>
              <Input id="role" placeholder="e.g., AI Developer, Project Manager" className="mt-1 bg-background/70 border-input focus:ring-primary h-9 text-sm" />
            </div>
        ...
        {currentStep === 3 && (
          <div className="space-y-4">
            ...
            <div>
              ...
              <Input id="agentName" placeholder="e.g., MyHelperBot" className="mt-1 bg-background/70 border-input focus:ring-primary h-9 text-sm" />
            </div>
            <div>
              <Label htmlFor="agentTask" className="text-xs">Initial Task</Label>
              <Input id="agentTask" placeholder="e.g., Summarize daily news, monitor email inbox" className="mt-1 bg-background/70 border-input focus:ring-primary h-9 text-sm" />
            </div>
        ...
        <div className="mt-6 md:mt-8 flex justify-between">
          <Button variant="outline" onClick={prevStep} disabled={currentStep === 1} className="bg-card/60 hover:bg-muted/60">
  ...
  const onboardingPageZoneConfigs: ZoneConfig[] = [
    {
      ...
      isDraggable: false, // Make it non-draggable for onboarding
      isResizable: false, // Make it non-resizable
      canPin: false,
      canMaximize: false,
      canMinimize: false,
      canClose: false, 
